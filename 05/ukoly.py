...
# 6
def sort_accort_second_letter(list1):
    podle_klice  = dict([])
    for zvire in list1:
        klic = zvire[1]
...
# sort_accort_second_letter(zvirata)

# 7
# rodne cislo 
def kontrola_rodne_cislo():
    rodne_cislo = input('Zadej rodne cislo: ')
    if not '/' in rodne_cislo or len(rodne_cislo) != 11:
        raise Exception("Nespravny format")
    else:
        # rodne_cislo = rodne_cislo.split('/')
        print('ok')

    cislo = ''
    for i, char in enumerate(rodne_cislo):
        if i == 6:
            if char != '/':
                return False
        else:
